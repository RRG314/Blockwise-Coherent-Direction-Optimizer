...
STRUCTURED_CORE_CANDIDATE_NAMES = (
    "gradient",
    "normalized_gradient",
    "stable_consensus",
    "trusted_direction",
...
STRUCTURED_CORE_COST_PENALTIES = {
    "gradient": 0.0,
...
STRUCTURED_CORE_STRESS_SPECIALISTS = {"projection_corrected", "orthogonal_escape", "sign_direction"}


class BCDOStructuredCore(BCDODirectionSelectionBase):
    """A sharper block-direction branch with explicit matrix consensus.

    This structured-core variant keeps the earlier trust-scored candidate
    framework, but narrows the default path toward the rules that survived the
    later internal branch study:
    ...
    candidate_names = STRUCTURED_CORE_CANDIDATE_NAMES
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        kwargs.setdefault("block_strategy", "smart_bcdo_structured")
        ...
        self._initialize_physical_optimizer("BCDOStructuredCore")

    @staticmethod
    def _resolve_block_strategy(param: torch.Tensor, strategy: str) -> str:
        if strategy == "smart_bcdo_structured":
            if param.ndim == 2:
                return "row"
            if param.ndim <= 1:
                return "scalar"
            return "row"
        return BCDODirectionSelectionBase._resolve_block_strategy(param, strategy)
        ...
        for group in self.param_groups:
            ...
            for param in group["params"]:
                ...
                for candidate_name in self.candidate_names:
                    ...
                    if candidate_name in STRUCTURED_CORE_STRESS_SPECIALISTS:
                    ...
                    raw_score = (
                        ...
                        - cost_penalty_weight * STRUCTURED_CORE_COST_PENALTIES[candidate_name]
                        - specialist_penalty
                    )
