...
from .bcdo_direction_selection_base import _normalize_rows, _row_cosine, _softmax_masked
from .blockwise_consensus_direction_optimizer import BCDO_COST_PENALTIES, BlockwiseConsensusDirectionOptimizer
from .optimizer_utils import average, clamp_scalar


class BCDOCNNReference(BlockwiseConsensusDirectionOptimizer):
    """CNN reference keeps BCDO mainline's core and uses conv structure as trust, not direction.

    The branch target is broader usability without losing the block branch's
    stress-task identity. Instead of adding another default candidate, CNN reference:

    - keeps the small BCDO mainline candidate set
    - keeps conv-safe scaling from the earlier conv-focused study
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self._initialize_physical_optimizer("BCDOCNNReference")
