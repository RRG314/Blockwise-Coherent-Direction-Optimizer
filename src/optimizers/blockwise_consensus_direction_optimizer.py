...
from .bcdo_direction_selection_base import (
    BCDODirectionSelectionBase,
    _normalize_rows,
    _row_cosine,
    _row_sign_flip_ratio,
    _softmax_masked,
)
from .bcdo_structured_core import BCDOStructuredCore
from .optimizer_utils import average, clamp_scalar


BCDO_CANDIDATE_NAMES = (
    "gradient",
    "stable_consensus",
    "trusted_direction",
    "low_rank_matrix",
...
class BlockwiseConsensusDirectionOptimizer(BCDOStructuredCore):
    """BCDO mainline with a reduced, structured candidate set.

    The accepted BCDO path keeps the novel part of the block branch: per-block candidate-direction
    ...
    candidate_names = BCDO_CANDIDATE_NAMES
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        kwargs.setdefault("block_strategy", "smart_bcdo")
        ...
        self._initialize_physical_optimizer("BlockwiseConsensusDirectionOptimizer")

    @staticmethod
    def _resolve_block_strategy(param: torch.Tensor, strategy: str) -> str:
        if strategy == "smart_bcdo":
            if param.ndim <= 1:
                return "tensor"
            ...
            if param.ndim >= 2:
                return "row"
            return "tensor"
        return BCDOStructuredCore._resolve_block_strategy(param, strategy)
    ...
    def _block_view_v4(self, tensor: torch.Tensor, strategy: str, small_matrix_cutoff: int) -> tuple[torch.Tensor, tuple[str, tuple[int, ...]]]:
        ...
        if strategy == "smart_bcdo":
            if tensor.ndim <= 1:
                ...
                resolved = "tensor"
        return BCDODirectionSelectionBase._block_view(tensor, resolved)
        ...
        for group in self.param_groups:
            ...
            for param in group["params"]:
                ...
                matrix_candidates = self._matrix_candidate_blocks(
                    grad,
                    "tensor" if block_strategy == "smart_bcdo" and grad.ndim == 2 and grad.numel() < small_matrix_cutoff else block_strategy,
                    eps,
                ...
                for candidate_name in self.candidate_names:
                    ...
                    raw_score = (
                        ...
                        - cost_penalty_weight * BCDO_COST_PENALTIES[candidate_name]
                    )
                ...
                param.add_(BCDODirectionSelectionBase._restore_blocks(update_blocks, layout), alpha=1.0)
                ...
                state["trusted_direction"] = BCDODirectionSelectionBase._restore_blocks(trusted_next, layout)
                state["smoothed_direction"] = BCDODirectionSelectionBase._restore_blocks(smoothed_next, layout)
                state["prev_grad"] = BCDODirectionSelectionBase._restore_blocks(descent_blocks, layout)
                state["prev_update"] = BCDODirectionSelectionBase._restore_blocks(update_blocks, layout)
