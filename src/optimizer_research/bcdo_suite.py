...
DEFAULT_OUTPUT_DIR = PROJECT_ROOT / "reports" / "bcdo_mainline"

FOCUS_OPTIMIZERS = [
    "blockwise_consensus_direction_optimizer",
    "blockwise_consensus_direction_optimizer_legacy",
    "bcdo_cnn_reference",
...
def bcdo_default_config() -> dict[str, Any]:
    return {
        ...
        "smoke_optimizers": ["blockwise_consensus_direction_optimizer", "blockwise_consensus_direction_optimizer_legacy", "adamw", "rmsprop"],
        "smoke_seeds": [11],
...
def _prepare_docs(output_dir: Path) -> None:
    write_bcdo_current_state(output_dir)
    write_bcdo_literature_scan(output_dir)
    write_bcdo_math_definition(output_dir)


def write_bcdo_current_state(output_dir: str | Path = DEFAULT_OUTPUT_DIR) -> None:
...
def write_bcdo_literature_scan(output_dir: str | Path = DEFAULT_OUTPUT_DIR) -> pd.DataFrame:
    output_path = Path(output_dir)
    output_path.mkdir(parents=True, exist_ok=True)
    frame = pd.DataFrame(LITERATURE_ROWS)
    frame.to_csv(output_path / "literature_matrix.csv", index=False)
    lines = [
        "# Blockwise Consensus Direction Optimizer Literature Scan",
        "",
        ...
        "- SAM remains the most relevant perturbation-trust baseline, but BCDO does not perturb the objective itself.",
        ...
        "- The accepted fast internal path is specifically testing whether typed tensor profiles and cheap structure-aware step control improve practicality without collapsing the branch into Adam-like logic.",
        "",
    ]
...
def write_bcdo_math_definition(output_dir: str | Path = DEFAULT_OUTPUT_DIR) -> None:
    output_path = Path(output_dir)
    output_path.mkdir(parents=True, exist_ok=True)
    lines = [
...
def run_bcdo_smoke(config: dict[str, Any]) -> pd.DataFrame:
    output_dir = ensure_output_dir(config)
    _prepare_docs(output_dir)
    return run_smoke_suite(config)


def run_bcdo_tuning(config: dict[str, Any]) -> pd.DataFrame:
    output_dir = ensure_output_dir(config)
    _prepare_docs(output_dir)
...
def _write_win_flags(output_path: Path, aggregated: pd.DataFrame) -> pd.DataFrame:
    ...
    for baseline_name in BASELINE_COMPARISONS:
        if baseline_name not in set(aggregated["optimizer"]):
            continue
        wins = compute_meaningful_wins(aggregated, "blockwise_consensus_direction_optimizer", baseline_name)
        if not wins.empty:
            frames.append(wins)
...
def run_bcdo_benchmarks(config: dict[str, Any]) -> pd.DataFrame:
    output_dir = ensure_output_dir(config)
    _prepare_docs(output_dir)
...
def run_bcdo_ablation(config: dict[str, Any]) -> pd.DataFrame:
    output_dir = ensure_output_dir(config)
    _prepare_docs(output_dir)
    ...
    variants = [
        {"variant_name": "v4_fast_full", "optimizer_name": "blockwise_consensus_direction_optimizer", "overrides": {}},
        {"variant_name": "v4_fast_legacy", "optimizer_name": "blockwise_consensus_direction_optimizer_legacy", "overrides": {}},
        {"variant_name": "v42_baseline", "optimizer_name": "bcdo_cnn_reference", "overrides": {}},
        {"variant_name": "no_conv_profile_split", "optimizer_name": "blockwise_consensus_direction_optimizer", "overrides": {"use_typed_profiles": False}},
        {"variant_name": "no_conv_structure_support", "optimizer_name": "blockwise_consensus_direction_optimizer", "overrides": {"use_conv_structure_support": False, "conv_energy_power_bonus": 0.0, "conv_step_floor": 1.0}},
        {"variant_name": "no_conv_support_bonus", "optimizer_name": "blockwise_consensus_direction_optimizer", "overrides": {"conv_consensus_bonus": 0.0, "conv_memory_bonus": 0.0}},
        {"variant_name": "no_conv_fallback_relaxation", "optimizer_name": "blockwise_consensus_direction_optimizer", "overrides": {"conv_fallback_relaxation": 0.0}},
        {"variant_name": "no_stable_consensus", "optimizer_name": "blockwise_consensus_direction_optimizer", "overrides": {"use_stable_consensus_candidate": False, "stable_consensus_bonus": 0.0}},
        {"variant_name": "no_trusted_direction", "optimizer_name": "blockwise_consensus_direction_optimizer", "overrides": {"use_trusted_direction_candidate": False, "coherence_weight": 0.0}},
        {"variant_name": "no_matrix_consensus", "optimizer_name": "blockwise_consensus_direction_optimizer", "overrides": {"use_low_rank_candidate": False, "matrix_consensus_bonus": 0.0}},
        {"variant_name": "no_block_structure", "optimizer_name": "blockwise_consensus_direction_optimizer", "overrides": {"block_strategy": "tensor"}},
        {"variant_name": "recoverability_periodic", "optimizer_name": "blockwise_consensus_direction_optimizer", "overrides": {"use_recoverability_gate": True, "recoverability_weight": 0.10, "recoverability_interval": 8}},
...
def export_bcdo_report(output_dir: str | Path = DEFAULT_OUTPUT_DIR) -> dict[str, Any]:
    output_path = Path(output_dir)
    ...
    legacy_row = _best_row_for_optimizer(aggregated, "blockwise_consensus_direction_optimizer_legacy")
    ...
    win_map = {
        baseline: compute_meaningful_wins(aggregated, "blockwise_consensus_direction_optimizer", baseline)
    ...
    _plot_bar(ablation_summary, figure_dir / "ablation_chart.png", "variant_name", "mean_selection_score", "BCDO ablation", 45)
    ...
    family_summary = (
        benchmark_frame[benchmark_frame["optimizer"] == "blockwise_consensus_direction_optimizer"]
    ...
    lines = [
        ...
        "## 3. Best rows",
        "",
        f"- Best BCDO row: `{v4_row['task']}` with mean best val loss `{float(v4_row['mean_best_val_loss']):.6f}` and mean best val accuracy `{float(v4_row['mean_best_val_accuracy']):.6f}`." if v4_row is not None else "- Best BCDO row: unavailable.",
        f"- Best legacy fast-path row: `{legacy_row['task']}` with mean best val loss `{float(legacy_row['mean_best_val_loss']):.6f}` and mean best val accuracy `{float(legacy_row['mean_best_val_accuracy']):.6f}`." if legacy_row is not None else "- Best legacy fast-path row: unavailable.",
        f"- Best reference CNN-branch row: `{v42_row['task']}` with mean best val loss `{float(v42_row['mean_best_val_loss']):.6f}` and mean best val accuracy `{float(v42_row['mean_best_val_accuracy']):.6f}`." if v42_row is not None else "- Best reference CNN-branch row: unavailable.",
        ...
        "",
        f"- BCDO wins vs the legacy fast path: `{v4_vs.get('blockwise_consensus_direction_optimizer_legacy', 0)}` meaningful wins.",
        f"- BCDO wins vs the reference CNN branch: `{v4_vs.get('bcdo_cnn_reference', 0)}` meaningful wins.",
        f"- BCDO wins vs AdamW: `{v4_vs.get('adamw', 0)}` meaningful wins; tracked 2x wins `{v4_two_x.get('adamw', 0)}`.",
        f"- BCDO wins vs RMSProp: `{v4_vs.get('rmsprop', 0)}` meaningful wins.",
        f"- BCDO wins vs SGD momentum: `{v4_vs.get('sgd_momentum', 0)}` meaningful wins.",
        f"- BCDO wins vs the internal coherent momentum comparator family: `{v4_vs.get('magneto_hamiltonian_adam', 0)}` meaningful wins.",
        "",
        "## 5. Runtime",
        "",
        f"- Mean BCDO runtime per step: `{float(v4_row['mean_runtime_per_step_ms']):.4f} ms`." if v4_row is not None else "- Mean BCDO runtime per step: unavailable.",
        f"- Mean legacy fast-path runtime per step: `{float(legacy_row['mean_runtime_per_step_ms']):.4f} ms`." if legacy_row is not None else "- Mean legacy fast-path runtime per step: unavailable.",
        f"- Mean reference CNN-branch runtime per step: `{float(v42_row['mean_runtime_per_step_ms']):.4f} ms`." if v42_row is not None else "- Mean reference CNN-branch runtime per step: unavailable.",
        "",
        "## 6. Ablation findings",
        "",
        ...
        "## 7. Honest conclusion",
        "",
        f"- Best task family for BCDO: `{best_family}`." if best_family is not None else "- Best task family for BCDO: unavailable.",
        ...
        "- If RMSProp or SGD momentum still dominate broadly, BCDO remains a specialist optimizer rather than a new default.",
        "",
    ]
