# Reference CNN Branch Literature Scan
...
The reference CNN branch keeps the same literature pressure as the accepted BCDO line: it only matters if structure-aware blockwise direction choice beats simpler gradient transforms in the places where structure actually matters.
...
## Reference-branch interpretation

- This branch is still different from [AdamW](../../REFERENCES.md#adam-and-adamw) and [RMSProp](../../REFERENCES.md#rmsprop) because the direction is selected blockwise rather than transformed by per-parameter moments.
- Compared with [Muon](../../REFERENCES.md#muon), [Shampoo](../../REFERENCES.md#shampoo), and [K-FAC](../../REFERENCES.md#k-fac), the new idea is lightweight conv structure as a trust signal rather than a matrix preconditioner.
- Compared with the earlier conv-safe branch, the conv structure signal no longer claims to generate a better direction directly; it only influences trust and fallback.
