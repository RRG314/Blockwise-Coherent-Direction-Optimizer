...
from optimizer_research.bcdo_suite import (  # noqa: E402
    bcdo_default_config,
    export_bcdo_report,
    run_bcdo_ablation,
    run_bcdo_benchmarks,
...
from optimizers import BlockwiseConsensusDirectionOptimizer  # noqa: E402
    ...
    config.update(
        {
            ...
            "smoke_optimizers": ["blockwise_consensus_direction_optimizer", "adamw"],
...
def test_bcdo_initializes_and_steps() -> None:
    set_global_seed(123)
    model = TinyRegressor()
    optimizer = BlockwiseConsensusDirectionOptimizer(model.parameters(), lr=2e-3)
...
def test_bcdo_state_dict_round_trip() -> None:
    set_global_seed(124)
    model = TinyRegressor()
    optimizer = BlockwiseConsensusDirectionOptimizer(model.parameters(), lr=2e-3)
    criterion = torch.nn.MSELoss()
    ...
    optimizer_2 = BlockwiseConsensusDirectionOptimizer(model_2.parameters(), lr=2e-3)
    optimizer_2.load_state_dict(state)
    assert optimizer_2.state_dict()["state"]
    assert optimizer_2.latest_diagnostics() == {"optimizer": "BlockwiseConsensusDirectionOptimizer"}


def test_bcdo_no_nans_after_multiple_steps() -> None:
    set_global_seed(125)
    model = TinyRegressor()
    optimizer = BlockwiseConsensusDirectionOptimizer(model.parameters(), lr=2e-3)
    criterion = torch.nn.MSELoss()
...
def test_bcdo_runs_on_conv_filters() -> None:
    set_global_seed(456)
...
def test_bcdo_runs_on_cuda_if_available() -> None:
...
def test_bcdo_suite_outputs(tmp_path: Path) -> None:
    config = _minimal_suite_config(tmp_path)
    write_bcdo_current_state(tmp_path)
    write_bcdo_literature_scan(tmp_path)
    write_bcdo_math_definition(tmp_path)
    smoke = run_bcdo_smoke(config)
    tuning = run_bcdo_tuning(config)
    bench = run_bcdo_benchmarks(config)
    ablation = run_bcdo_ablation(config)
    export_bcdo_report(tmp_path)
...
def test_bcdo_mps_step_if_available() -> None:
